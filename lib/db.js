...
const { defaults, mergeDefaults } = require("./content");
...
const _getStmt = db.prepare("SELECT value FROM content WHERE key = ?");
...
function addAuditEntry(user, action, detail) {
  ...
  db.prepare("DELETE FROM audit_log WHERE id NOT IN (SELECT id FROM audit_log ORDER BY id DESC LIMIT 500)").run();
}
