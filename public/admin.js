  ...
  DATA.services.forEach((s, i) => {
    ...
    el.innerHTML = `
      <div class="row-head"><span class="grip">☰</span>
        <select>${ICON_OPTIONS.map((o) => `<option value="${o}" ${o === s.icon ? "selected" : ""}>${o}</option>`).join("")}</select>
    ...
    el.querySelector("select").addEventListener("change", (e) => { s.icon = e.target.value; show("contentSaveBar"); });
